"""
tests.verify_full_application - Rigorous Full Application End-to-End Test Suite
"""
...
from app.service import SysOpsRAGService

def run_full_application_tests():
    print("=" * 70)
    print("RUNNING RIGOROUS FULL APPLICATION END-TO-END VERIFICATION")
    print("=" * 70)
    
    service = SysOpsRAGService()
    
    # TEST 1: Cross-Contamination Prevention
    ...
    assert has_redis and not has_jwt_contamination, "Test 1 Failed."
    ...
    assert has_sop211 and has_user_ids, "Test 2 Failed."
    ...
    assert has_sop212, "Test 3 Failed."
    print("[PASS] Test 3: PostgreSQL Long-Running Query Diagnosis Verified.")

    # TEST 4: Full 4,600+ Line Log Search Engine Test
    print("\n[TEST 4] Full-File Log Search Engine Test (Line 366 Search)")
    log_path = os.path.join(PROJECT_ROOT, "data", "logs", "auth_service", "auth_errors.log")
    ...
    assert found_target, "Test 4 Failed."
    ...
    assert res5["guardrail_triggered"] == True, "Test 5 Failed."
