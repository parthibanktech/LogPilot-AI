"""
test_live_http_api.py - Live HTTP Server API Verification Suite
"""
...
def test_live_http_endpoints():
    ...
    try:
        ...
        assert r1.json().get("status") == "healthy", "Health status not healthy."
    ...
    try:
        ...
        logs_text = r2.json().get("logs", "")
        found = "usr_54272" in logs_text
        print(f"Found 'usr_54272' in Search Response: {found}")
        assert found, "Log search API did not find target user ID."
    ...
    try:
        payload3 = {
            "query": "JWT Signature verification failed: Token expired",
            "service_filter": "auth_service"
        }
        t0 = time.time()
        r3 = requests.post(f"{BASE_URL}/api/query", json=payload3, timeout=25)
        latency = (time.time() - t0) * 1000
        print(f"Status Code: {r3.status_code} | Latency: {latency:.0f}ms")
        data3 = r3.json()
        ans3 = data3.get("answer", "")
        
        has_sop211 = "SOP-211" in ans3 or "jwt" in ans3.lower()
        print(f"SOP-211 Cited: {has_sop211}")
        assert r3.status_code == 200 and has_sop211, "Auth query API failed."
    ...
    try:
        payload4 = {
            "query": "Redis cluster reports OOM command not allowed when used memory > maxmemory. What is the resolution?",
            "service_filter": "auth_service"
        }
        t0 = time.time()
        r4 = requests.post(f"{BASE_URL}/api/query", json=payload4, timeout=25)
        latency = (time.time() - t0) * 1000
        print(f"Status Code: {r4.status_code} | Latency: {latency:.0f}ms")
        data4 = r4.json()
        ans4 = data4.get("answer", "")
        
        has_sop202 = "SOP-202" in ans4 or "maxmemory" in ans4.lower()
        has_jwt_contam = "jwt signature verification failed" in ans4.lower()
        
        print(f"Redis SOP-202 Cited: {has_sop202}")
        print(f"Zero Auth Contamination: {not has_jwt_contam}")
        assert r4.status_code == 200 and has_sop202 and not has_jwt_contam, "Redis query API failed."
    ...
    try:
        ...
        print(f"Telemetry Summary: {r5.json()}")
        assert r5.status_code == 200, "Telemetry failed."
